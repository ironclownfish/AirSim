    ...
    : api_provider_(api_provider)
{
	...
	pimpl_->server.bind("getLastFullLidarScan", [&](const std::string& lidar_name, const std::string& vehicle_name) -> RpcLibAdapatorsBase::LidarData {
		const auto& lidar_data = getVehicleApi(vehicle_name)->getLidarData(lidar_name, true);
		return RpcLibAdapatorsBase::LidarData(lidar_data);
	});
